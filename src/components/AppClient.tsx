...
export default function AppClient({ email, initialCredits }: Props) {
  ...
  function createLogoFor(name: string) {
    setSeedName(name);
    setView("brand");
    setShowPacks(false);
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-5xl px-6 py-8">
        {/* Tabs */}
        <div className="mb-8 inline-flex gap-1 rounded-xl border border-white/10 bg-white/5 p-1">
          {tab("find", "🔎 Find names")}
          {tab("brand", "🎨 Design a brand")}
        </div>
        ...
        {view === "brand" ? (
          <LogoStudio
            ...
            onNeedCredits={() => setShowPacks(true)}
            seedName={seedName}
          />
        ) : (
          <>
            ...
            <p className="mt-2 text-slate-400">
              Describe your company and we&apos;ll brainstorm names, then check
              each one against the live web. Each search costs {NAME_CREDIT_COST}{" "}
              credit.
            </p>
              ...
              <div className="mt-10 space-y-10">
                <section>
                  ...
                  <p className="mb-4 text-sm text-slate-500">
                    ...
                    Like one? Design its logo in a click.
                  </p>
                  {available.length > 0 ? (
